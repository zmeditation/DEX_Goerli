...
// 40 EBIT is minted per block
// 20 EBIT per block is sent to Burn pool (A farm just for burning cake)
// 10 EBIT per block goes to EBIT syrup pool
// 9 EBIT per block goes to Yield farms and lottery
// COLA_PER_BLOCK in config/index.ts = 40 as we only change the amount sent to the burn pool which is effectively a farm.
// EBIT/Block in src/views/Home/components/CakeStats.tsx = 19 (40 - Amount sent to burn pool)
